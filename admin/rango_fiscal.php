...
<div class="box-body">
<?php 
$consulta=("SELECT * FROM o_r WHERE habilitado=3 AND fecha_cierre BETWEEN date('$fecha1') AND date('$fecha2')");
$s=cons($consulta);
...
$id_carro=$e['id_carro'];
echo '<td><a href="detalles_or_a.php?id_or='.$id_or.'">'.$id_or.'</a></td>';
echo '<td><a href="detalles_or_a.php?id_or='.$id_or.'">'.$numero_or.'</a></td>';
...
while($row2=mysqli_fetch_array($s2)){
  if ($fecha1 < $fecha_recor) {
 $monto11=$row2['monto'];  

  }else{
    $monto11=($row2['monto']*100000);
  }
...
<td>'.$cliente.'</td>
<td>'.number_format($tiva,2,",",".").'&nbsp;Bs || '.number_format($tiva/100000,2,",",".").' Bs S.</td>
<td>'.number_format($tmonto,2,",",".").'&nbsp;Bs || '.number_format($tmonto/100000,2,",",".").' Bs S.</td>
</tr>'; 
...
<div><br></div>
<div class="alert alert-warning alert-dismissible des">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>Recordatorio:
              <p>Desde el 20 de Agosto 2018 los montos pasaron a Bol&iacute;vares Soberanos</p></div> 
<style>
#totales{
position: fixed;
bottom: 0px;
...
$fe=explode("-",$fecha2);
$fecha2=$fe[2]."-".$fe[0]."-".$fe[1];
$fecha2_1=$fe[0]."-".$fe[1]."-".$fe[2];
$consulta34=("SELECT * FROM o_r WHERE habilitado=3 AND fecha_cierre BETWEEN date('$fecha1') AND date('$fecha2')");
$f34=cons($consulta34);
$todo=0;
$total_or=0;
$total_servicios=0;
...
$cant=$ro['cantidad'];
$prec=$ro['precio'];
  if ($fecha2_1 < $fecha_recor) {
 $prec=$ro['precio'];  
  }else{
    $prec=($ro['precio']*100000);
  }
$taller=$ro['taller'];
if($taller==0){
...
while($ros=mysqli_fetch_array($r)){
 ...
 $servicios=$ros['monto'];  
    ...
    $servicios=($ros['monto']*100000);
  }
$total_servicios=($total_servicios+$servicios);
    }
...
$total_fac_venta=bcdiv($total_monto, '100000', 3);
$total_iva_venta=bcdiv($total_iva, '100000', 3);
$total_or_todo=bcdiv($todo, '100000', 3);
$total_iva_or_todo=bcdiv($todo2_iva, '100000', 3);
...
IVA:&nbsp;'.number_format($total_iva,2,",",".").'&nbsp;Bs</h5>

<h5>Total Ordenes:&nbsp;'.$cuenta_ordenes.'<br>
Facutrado en Ordenes:&nbsp;'.number_format($todo,2,",",".").'&nbsp;Bs<br>
IVA:&nbsp;'.number_format($todo2_iva,2,",",".").'&nbsp;Bs</h5><hr border="1px" size="20%">
Reconversi&oacute;n Monetaria:&nbsp;<br>
<h5>Total Facturado por Ventas:&nbsp;'.number_format(($total_fac_venta),3,",",".").' Bs. S.<br>
IVA:&nbsp;'.number_format(($total_iva_venta),3,",",".").'&nbsp;Bs</h5>
<h5>Total Ordenes:&nbsp;'.$cuenta_ordenes.'<br>
Facutrado en Ordenes:&nbsp;'.number_format(($total_or_todo),3,",",".").'&nbsp;Bs S.<br>
IVA:&nbsp;'.number_format(($total_iva_or_todo),3,",",".").'&nbsp;Bs S.</h5>
</div>';
?>
<!-- date-range-picker -->
